import React from 'react';
import style from './HeaderFinal.module.css';

import {
  faUserCircle,
  faCaretRight,
  faCaretDown,
  faSignOutAlt,
} from '@fortawesome/free-solid-svg-icons';
import { UserContext } from '../Contexts/UserContext';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import Follow from './../Components/RedesSociais/Follow';

const HeaderFinal = () => {
  const { user, userLogout, login } = React.useContext(UserContext);

  return (
    <div className="App">
      <nav>
        <div className={style.navbar}>
          <div className={[style.conatiner, style.nav_container].join(' ')}>
            <label for="chkHamburger" style={{ color: '#F7F7F7' }}>
              .
            </label>
            <input
              role="presentation"
              className={style.checkbox}
              type="checkbox"
              name="chkHamburger"
              id="chkHamburger"
            />

            <div className={style.hamburger_lines}>
              <span className={[style.line, style.line1].join(' ')}></span>
              <span className={[style.line, style.line2].join(' ')}></span>
              <span className={[style.line, style.line3].join(' ')}></span>
            </div>
            <div className={style.logo}>
              <a href="/" rel="dofollow">
                <img
                  loading="auto"
                  style={{
                    width: '130px',
                    height: 'auto',
                    position: 'absolute',
                    top: '8px',
                    right: '15px',
                  }}
                  // src={fgLogo}
                  src="data:image/png;base64,"
                  alt="Logotipo Site Guardar Dinheiro"
                />
              </a>
            </div>
            <div className={style.menu_items}>
              <ul>
                {login ? (
                  <li>
                    <a
                      href="#"
                      style={{
                        paddingLeft: '20px',
                        fontSize: '22px',
                        // color: 'white',
                      }}
                      // style={{ textDecoration: 'underline', textAlign: 'center' }}
                    >
                      <FontAwesomeIcon
                        icon={faUserCircle}
                        style={{
                          fontSize: '30px',
                          // color: 'white',
                        }}
                      />
                      {' ' + user}
                    </a>
                  </li>
                ) : null}
                {login ? (
                  <li>
                    <a
                      // href="/meu-portfolio"
                      href="/favoritos"
                      style={{ textDecoration: 'underline' }}
                    >
                      Meus Investimentos Favoritos
                    </a>{' '}
                    {'   /   '}
                    <a href="#" onClick={userLogout}>
                      {'Sair '}
                      <FontAwesomeIcon icon={faSignOutAlt} />
                    </a>
                    {/* <hr /> */}
                  </li>
                ) : (
                  <li>
                    <a
                      href="/autenticacao"
                      style={{
                        paddingLeft: '10px',
                        fontSize: '18px',
                        textDecoration: 'underline',
                      }}
                    >
                      <FontAwesomeIcon icon={faUserCircle} /> {' Login'}
                    </a>{' '}
                    {'   /   '}
                    <a
                      href="/autenticacao/criar"
                      style={{
                        fontSize: '18px',
                        textDecoration: 'underline',
                      }}
                    >
                      {' Cadastre-se Grátis'}
                    </a>
                    {/* <hr /> */}
                  </li>
                )}
                <li>
                  <a href="/fundos-de-investimento" rel="dofollow">
                    Melhores Fundos de Investimentos
                  </a>
                </li>

                <li>
                  <a href="/fundos-imobiliarios" rel="dofollow">
                    Melhores Fundos Imobiliários
                  </a>
                </li>

                <li>
                  <a href="/acoes" rel="dofollow">
                    Melhores Ações
                  </a>
                </li>
                <li>
                  <a href="/etf" rel="dofollow">
                    Melhores ETFs
                  </a>
                </li>
                <li>
                  <a href="/bdr" rel="dofollow">
                    Melhores BDRs
                  </a>
                </li>
                <li>
                  <a href="/criptomoeda" rel="dofollow">
                    Melhores Criptomoedas
                  </a>
                </li>
                <li>
                  <a href="/calculadora-renda-fixa" rel="dofollow">
                    Calculadora de Renda Fixa
                  </a>
                </li>
                <li>
                  <a href="/como-juntar/1000000" rel="dofollow">
                    Calculadora do Milhão
                  </a>
                </li>

                <li>
                  <a href="/emprestimo/0/simulador" rel="dofollow">
                    Simulador de Empréstimo
                  </a>
                </li>
                <li>
                  <a href="/melhor-cartao-credito" rel="dofollow">
                    Melhor Cartão de Crédito
                  </a>
                </li>
              </ul>
              <div>
                <ul className={style.ul_collapse}>
                  <li>
                    <input type="checkbox" id="list-item-1" />
                    <label for="list-item-1" className={style.first}>
                      {'Quanto Rende Meu Dinheiro no(a) '}
                      <FontAwesomeIcon icon={faCaretDown} />
                    </label>
                    <ul className={style.ul_collapse}>
                      <li>
                        <a href="/quanto-rende/0/pagbank" rel="dofollow">
                          {'PagBank '} <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>

                      <li>
                        <a href="/quanto-rende/0/picpay" rel="dofollow">
                          {'PicPay '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/nubank" rel="dofollow">
                          {'Nubank '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/tesouro" rel="dofollow">
                          {'Tesouro Direto '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/poupanca" rel="dofollow">
                          {'Poupança '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/mercadopago" rel="dofollow">
                          {'Mercado Pago '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/neon" rel="dofollow">
                          {'Banco Neon '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/will" rel="dofollow">
                          {'Will Bank '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/bv" rel="dofollow">
                          {'Banco BV '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/iti" rel="dofollow">
                          {'Iti '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/next" rel="dofollow">
                          {'Banco Next '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/inter" rel="dofollow">
                          {'Banco Inter '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/99pay" rel="dofollow">
                          {'99Pay '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/quanto-rende/0/fgts" rel="dofollow">
                          {'FGTS '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                    </ul>
                  </li>
                  <li>
                    <input type="checkbox" id="list-item-2" />
                    <label for="list-item-2">
                      {'Comparadores de Investimentos '}
                      <FontAwesomeIcon icon={faCaretDown} />
                    </label>
                    <ul className={style.ul_collapse}>
                      <li>
                        <a
                          href="/comparador-fundos-de-investimentos"
                          rel="dofollow"
                        >
                          {'Fundos de Investimentos '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>

                      <li>
                        <a
                          href="/comparador-fundos-imobiliarios"
                          rel="dofollow"
                        >
                          {'Fundos Imobiliários '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/comparador-acoes" rel="dofollow">
                          {'Ações '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/comparador-etf" rel="dofollow">
                          {'ETFs '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/comparador-bdr" rel="dofollow">
                          {'BDRs '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/comparador-cripto" rel="dofollow">
                          {'Criptomoedas '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                      <li>
                        <a href="/calculadora-renda-fixa" rel="dofollow">
                          {'Renda Fixa '}
                          <FontAwesomeIcon icon={faCaretRight} />
                        </a>
                      </li>
                    </ul>
                  </li>
                  <li>
                    <input type="checkbox" id="list-item-4" />
                    <label for="list-item-4">
                      {'Mais Acessados'}
                      <FontAwesomeIcon icon={faCaretDown} />
                    </label>
                    <ul className={style.ul_collapse}>
                      <li>
                        <a href="/cripto/btc" rel="dofollow">
                          BITCOIN
                        </a>
                      </li>
                      <li>
                        <a href="/cripto/eth" rel="dofollow">
                          ETHEREUM
                        </a>
                      </li>
                      <li>
                        <a href="/acoes/petr4" rel="dofollow">
                          PETR4
                        </a>
                      </li>
                      <li>
                        <a href="/acoes/vale3" rel="dofollow">
                          VALE3
                        </a>
                      </li>
                      <li>
                        <a href="/acoes/mglu3" rel="dofollow">
                          MGLU3
                        </a>
                      </li>
                      <li>
                        <a href="/fii/mxrf11" rel="dofollow">
                          MXRF11
                        </a>
                      </li>
                      <li>
                        <a href="/bdr/m1ta34" rel="dofollow">
                          M1TA34
                        </a>
                      </li>
                      <li>
                        <a href="/etf/bova11" rel="dofollow">
                          BOVA11
                        </a>
                      </li>
                    </ul>
                  </li>
                  <li>
                    <input type="checkbox" id="list-item-3" />
                    <label for="list-item-3">
                      {'Não sei investir '}
                      <FontAwesomeIcon icon={faCaretDown} />
                    </label>
                    <ul className={style.ul_collapse}>
                      <li>
                        <a href="/educacao-financeira" rel="dofollow">
                          Comece por Aqui
                        </a>
                      </li>
                      <li>
                        <a href="/livros" rel="dofollow">
                          Livros Recomendados
                        </a>
                      </li>
                    </ul>
                  </li>
                </ul>
              </div>

              {/* <hr /> */}
              <Follow header={true} />
              {/* <p style={{ color: '#0e2431', marginLeft: '20px' }}>
                Redes Sociais
              </p>
              <div className={style.faixa}>
                <ul>
                  <li>
                    <a
                      href="https://www.facebook.com/siteguardardinhero/"
                      title="Facebook Guardar Dinheiro"
                    >
                      <span>
                        <FontAwesomeIcon icon={faFacebookSquare} />{' '}
                      </span>
                    </a>
                  </li>
                  <li>
                    <a
                      href="https://www.instagram.com/guardardinheiro/"
                      title="Instagram Guardar Dinheiro"
                    >
                      <span>
                        {' '}
                        <FontAwesomeIcon icon={faInstagramSquare} />{' '}
                      </span>
                    </a>
                  </li>
                  <li>
                    <a
                      href="https://twitter.com/guardardinheir1"
                      title="Twitter Guardar Dinheiro"
                    >
                      <span>
                        <FontAwesomeIcon icon={faTwitterSquare} />
                      </span>
                    </a>
                  </li>
                  <li>
                    <a
                      href="https://t.me/+6JN2oQW-M5BlMTlh"
                      title="Telegram Guardar Dinheiro"
                    >
                      <span>
                        <FontAwesomeIcon icon={faTelegram} />{' '}
                      </span>
                    </a>
                  </li>
                  <li>
                    <a
                      href="https://blog.guardardinheiro.com.br/fale-conosco/"
                      title="E-mail Guardar Dinheiro"
                    >
                      <span>
                        {' '}
                        <FontAwesomeIcon icon={faEnvelope} />{' '}
                      </span>
                    </a>
                  </li>
                </ul>
              </div> */}
            </div>
          </div>
        </div>
      </nav>
    </div>
  );
};
export default HeaderFinal;
